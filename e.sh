...
eval $(ps -aux --sort=-%cpu -h | awk  -v num=0 \
	'{if($3 < 2) {exit} else {num++;printf("cpuproid["num"]=%d;",$2);} } END {printf("cpunum=%d;",num)}')
eval $(ps -aux --sort=-%mem -h | awk  -v num=0 \
	'{if($4 < 2) {exit} else {num++;printf("memproid["num"]=%d;",$2);} } END {printf("memnum=%d;",num)}')
...
if [[ $cpunum -gt 0 ]]; then
	...
	for i in ${cpuproid[*]}; do
		...
		eval $(ps -aux -q $i -h  | awk  -v num=$count \
			'{if($3 < 2) {exit} else {printf("cpuproname["num"]=%s;\
...
if [[ $memnum -gt 0 ]]; then
	...
	for i in ${memproid[*]}; do
		count=$[ $count + 1 ]
		eval $(ps -aux -q $i -h  | awk  -v num=$count \
			'{if($4 < 2) {exit} else {printf("memproname["num"]=%s;memproid["num"]=%d;memprouser["num"]=%s;memprocpu["num"]=%s;mempromem["num"]=%s;"\
...
if [[ $cpuchecknum -gt 0 ]]; then
	for (( i = 1; i <= $cpuchecknum; i++ )); do
		echo "$NowTime ${cpuproname[$i]} ${cpuproid[$i]} \
		${cpuprouser[$i]} ${cpuprocpu[$i]}% ${cpupromem[$i]}%"
	done
fi
