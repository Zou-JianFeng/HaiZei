...
int main() {
	...
	while (~fscanf(fpp, "%s", buffer)) {
		...
		for (int i = 0; buffer[i]; ++i) {
			...
			//printf("TCN : %s str : %s", TCN, huffman_code[str[i]]);
		}
	...
	// 	for (int i = 0; str[i]; ++i) {
	// 		word_cnt += 1;
	// 		strcat(TCN, huffman_code[str[i]]); //获取输入字符串的哈夫曼路径
	// 		//printf("TCN : %s str : %s", TCN, huffman_code[str[i]]);
	// 	}
	// 	printf("%s\n", TCN);
	// 	root = insert(root, TCN); //将路径插入二叉字典树中
	...
	while (n--) {
		...
		char TCN[MAX_LEN] = {0};
		for (int i = 0; str[i]; ++i) {
			strcat(TCN, huffman_code[str[i]]); //获取输入待查字符串的哈夫曼路径
		}
		//printf("%s\n", TCN);
		printf("%s\n", search(root, TCN) == 1 ? "已查到" : "未查到" );
	}
	clear(root);
	// printf("%d %d\n", word_cnt, node_cnt);
